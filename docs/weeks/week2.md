...
- **`size_t`란?**
  - `sizeof` 연산자의 반환 타입으로, C언어에서 객체의 크기를 표현하는 부호 없는 정수 타입.
  - 시스템의 메모리 모델에 따라 크기가 달라지며(32비트 시스템에서는 32비트, 64비트에서는 64비트), 어떤 객체든 그 크기를 담을 수 있을 만큼 충분히 큼.
  - `size_t` 타입의 값을 출력할 때는 `%zu` 서식 지정자를 사용해야 합니다.
...
- **Working Tree**: 현재 작업 중인 파일들이 있는 실제 디렉토리. 수정했지만 아직 커밋되지 않은 변경사항을 포함한다.
- **Index (Stage)**: 다음 커밋에 포함될 변경사항들의 스냅샷이 저장되는 임시 영역. `git add` 명령을 사용해 Working Tree의 변경사항을 Index에 추가할 수 있다.
- **HEAD**: 현재 브랜치의 가장 마지막 커밋을 가리키는 포인터. 저장소의 현재 버전을 나타낸다.
...
```
`git diff`: 작업 디렉토리(Working Tree)와 스테이징 영역(Index) 사이의 차이점을 보여준다. (아직 add하지 않은 변경사항)
`git diff --staged`: 스테이징 영역(Index)과 마지막 커밋(HEAD) 사이의 차이점을 보여준다. (add는 했지만 아직 commit하지 않은 변경사항)
